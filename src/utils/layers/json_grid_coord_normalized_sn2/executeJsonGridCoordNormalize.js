...
/**
 * 「JSON·網格·座標正規化」圖層：單鍵 **b→c→d**（見 `layerId`：'json_grid_coord_normalized_sn2'）。
...
import { LAYER_ID as OSM_2_LAYER_ID } from '@/utils/layers/osm_2_geojson_2_json_sn2/sessionOsmXml.js';
import {
...
export function executeJsonGridCoordNormalize() {
  ...
  const layer = dataStore.findLayerById('json_grid_coord_normalized_sn2');
  if (!layer) {
    console.warn('executeJsonGridCoordNormalize：找不到圖層', 'json_grid_coord_normalized_sn2');
    return false;
  }
  ...
  layer.dashboardData = {
    ...
    sourceLayerId: 'json_grid_coord_normalized_sn2',
    routeSourceLayerId: resolved.fromExistingSn ? 'json_grid_coord_normalized_sn2' : OSM_2_LAYER_ID,
    coordNormalize: true,
    straightened: true,
    ...out.meta,
    topologyCheck,
  ...
  dataStore.saveLayerState(
    'json_grid_coord_normalized_sn2',
...
export function executeJsonGridNeighborTopologyFix() {
  ...
  if (!r.ok) {
    dataStore.saveLayerState('json_grid_coord_normalized_sn2', {
