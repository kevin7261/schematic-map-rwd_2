/**
 * 衍生圖層：`point_orthogonal_sn2` 自「座標正規化」複製 dataJson／jsonData；
 * 「站點與路線往中心聚集」兩種線網層優先自 `point_orthogonal_sn2` 複製；若尚無陣列則改讀「座標正規化」同一欄位（便於只開本層也能顯示）。
 * `orthogonal_toward_center_vh_draw_sn2` 僅鏡像 `orthogonal_toward_center_vh_sn2` 之 dataJson／geojson 供繪製；
 * `layout_network_grid_from_vh_draw_sn2` 自繪製層複製 **dataOSM**（並解析為 geojson 供網格檢視）。
 */
...
import { osmXmlStringToGeojsonData } from '@/utils/layers/osm_2_geojson_2_json_sn2/pipeline.js';
import {
...
/** 同組複本：與 {@link syncLayoutNetworkGridRoutesDataJsonFromVhDraw} 相同邏輯，綁定 `layout_network_grid_from_vh_draw_copy_sn2`（實作重複，不呼叫前者）。 */
...
/** 版面網絡網格_2：與 {@link syncLayoutNetworkGridRoutesDataJsonFromVhDraw} 相同邏輯，綁定 `layout_network_grid_from_vh_draw_2_sn2`。 */
...
/** `layout_network_grid_read_layout_data_json_2_sn2`：深拷自 `layout_network_grid_from_vh_draw_2_sn2`。 */
...
/** `layout_network_grid_read_layout_data_json_2_sn2`：若該層存在則深拷並 persist（不論可見與否，避免再次開啟時為過期快照）。 */
...
/** 「版面網絡網格」層若可見，自 `orthogonal_toward_center_vh_draw_sn2` 重複製 **dataOSM** 並 persist */
...
/** 版面網絡網格_2：若可見則自 `orthogonal_toward_center_vh_draw_sn2` 重複製並 persist（平行於 {@link refreshLayoutNetworkGridFromVhDrawIfVisible}）。 */
...
/** 同組複本：若可見則自 `orthogonal_toward_center_vh_draw_sn2` 重複製並 persist（平行於 {@link refreshLayoutNetworkGridFromVhDrawIfVisible}，實作重複）。 */
...
/** `orthogonal_toward_center_vh_draw_sn2`：自 VH 往中心層複製 dataJson（VH 層或其他鏡像更新後呼叫） */
...
/** 站點層寫入後，任一「往中心聚集」線網層開啟則自 `point_orthogonal_sn2` 重鏡像並 persist */
