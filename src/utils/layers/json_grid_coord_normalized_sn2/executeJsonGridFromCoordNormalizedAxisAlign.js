...
/**
 * 「站點移動水平垂直化」圖層（layerId：`point_orthogonal_sn2`）：格點路網四鄰橫豎化、刪空欄列等。
 ...
 * 重算 Section／Connect／Station、processedJson、dataOSM 並 persist（僅寫入 `point_orthogonal_sn2` 圖層）。
 *
...
export function executeJsonGridFromCoordNormalizedPruneEmptyGridLines() {
  ...
  try {
    ...
    console.error('刪空欄列（point_orthogonal_sn2）：匯出 processedJsonData 失敗', e);
  }
