/**
 * layout_network_grid_from_vh_draw_sn2：將 CSV／合成 traffic 對應之數值寫入 VH 繪製層 segment **起點側**站點物件。
 *
 ...
 * @param {*} drawLayer — orthogonal_toward_center_vh_draw_sn2 圖層
...
/** @param {unknown[]} rows — `layout_network_grid_from_vh_draw_copy_sn2` 黑點表列（就地排序並重編 `#`） */
...
/**
 * `layout_network_grid_from_vh_draw_copy_sn2` 專用 DataTable：每個路段 `segment.stations` 中段站（黑點）一列，
