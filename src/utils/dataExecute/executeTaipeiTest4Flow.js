/**
 * 空間網絡網格測試_4：taipei_sn4_a→…→sn4_g、sn4_g→sn4_h、…、sn4_k→sn4_l→sn4_m
 * （流程與測試_3 之 a3～m3 分檔複製；圖層 id 獨立，import @/utils/taipeiTest4）
 */
...
import { applyHVToE3SpaceNetwork } from '@/utils/taipeiTest4/applyHVToE3Network.js';
import { applyNShapeReduceAllToSpaceNetworkData } from '@/utils/taipeiTest4/applyNShapeReduceToSpaceNetwork.js';
import { applyFlipLReduceF3ToG3Network } from '@/utils/taipeiTest4/f3ToG3FlipLReduce.js';
import { pruneGridLinesWithoutConnectVertices } from '@/utils/taipeiTest4/f3ToG3PruneEmptyGridLines.js';
import { applyG3ZShapeRedInteriorAxisSnapToNetwork } from '@/utils/taipeiTest4/g3ZShapeRedInteriorAxisSnapToNetwork.js';
import {
  assertA3RowsForG3ToH3,
  mergeConnectSpansPlaceBlackStationsAndSplit,
} from '@/utils/taipeiTest4/g3ToH3PlaceBlackStationsFromA3Rows.js';
import {
...
import { splitFlatH3SegmentsAtBlackVerticesOnly } from '@/utils/taipeiTest4/h3ToI3SplitAtBlackVertices.js';
import { applyTaipeiL3BlackDotReductionWhileMinDiffLessThan } from '@/utils/taipeiL3BlackDotReductionStep.js';
import { resolveTaipeiSn4ASketchFeatureCollection } from '@/utils/resolveTaipeiSn4ASketchGeojson.js';

/** a3：GeoJSON → 路段 → 寫入 b3（async 與其他 execute 呼叫一致） */
export async function executeTaipeiTest4_A3_To_B3() {
  const dataStore = useDataStore();
  const a3 = dataStore.findLayerById('taipei_sn4_a');
  const b3 = dataStore.findLayerById('taipei_sn4_b');
  if (!b3) {
    console.warn('executeTaipeiTest4_A3_To_B3：找不到 taipei_sn4_b');
    return;
  }
  ...
  if (!geojsonForExport?.features?.length && a3) {
    try {
      ...
      if (geojsonForExport?.features?.length) {
        ...
        a3.geojsonData = JSON.parse(JSON.stringify(geojsonForExport));
      }
    } catch (e) {
      console.warn('無可用 GeoJSON（請載入檔案、OSM 複製或於「手繪（測試_4）」繪製折線）', e?.message || e);
      return;
    }
  }
  if (!geojsonForExport?.features?.length) {
    console.warn('無可用 GeoJSON（請載入 taipei_sn4_a 之 geojsonData 或手繪折線）');
    return;
  }
  ...
  if (!b3.visible) {
    b3.visible = true;
    dataStore.saveLayerState('taipei_sn4_b', { visible: true });
  }
}

/** b3：交叉點直線化 → 寫入 c3 */
export function executeTaipeiTest4_B3_To_C3() {
  const dataStore = useDataStore();
  const b3 = dataStore.findLayerById('taipei_sn4_b');
  const c3 = dataStore.findLayerById('taipei_sn4_c');
  if (!b3?.spaceNetworkGridJsonData?.length || !c3) {
    console.warn('executeTaipeiTest4_B3_To_C3：缺少 b3 路網或 c3 圖層');
    return;
  }
  ...
  if (!c3.visible) {
    c3.visible = true;
    dataStore.saveLayerState('taipei_sn4_c', { visible: true });
  }
}

/** c3：座標正規化 → 寫入 d3 */
export function executeTaipeiTest4_C3_To_D3() {
  const dataStore = useDataStore();
  const c3 = dataStore.findLayerById('taipei_sn4_c');
  const d3Layer = dataStore.findLayerById('taipei_sn4_d');
  if (!c3?.spaceNetworkGridJsonData?.length || !d3Layer) {
    console.warn('executeTaipeiTest4_C3_To_D3：缺少 c3 路網或 d3 圖層');
    return;
  }
  ...
  if (!d3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_d', { visible: true });
  }
}

/** d3：刪除無紅／藍（connect）之整欄／整列並壓縮座標 → 寫入 e3 */
export function executeTaipeiTest4_D3_To_E3() {
  const dataStore = useDataStore();
  const d3Layer = dataStore.findLayerById('taipei_sn4_d');
  const e3Layer = dataStore.findLayerById('taipei_sn4_e');
  if (!d3Layer?.spaceNetworkGridJsonData?.length || !e3Layer) {
    console.warn('executeTaipeiTest4_D3_To_E3：缺少 d3 路網或 e3 圖層');
    return;
  }
  ...
  if (!e3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_e', { visible: true });
  }
}

/** e3：對角線轉水平垂直（applyHVToE3Network）→ 寫入 f3 */
export function executeTaipeiTest4_E3_To_F3() {
  const dataStore = useDataStore();
  const e3Layer = dataStore.findLayerById('taipei_sn4_e');
  const f3Layer = dataStore.findLayerById('taipei_sn4_f');
  if (!e3Layer?.spaceNetworkGridJsonData?.length || !f3Layer) {
    console.warn('executeTaipeiTest4_E3_To_F3：缺少 e3 路網或 f3 圖層');
    return;
  }
  ...
  if (!f3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_f', { visible: true });
  }
}
 ...
 */
export function executeTaipeiTest4_F3_To_G3() {
  const dataStore = useDataStore();
  const f3Layer = dataStore.findLayerById('taipei_sn4_f');
  const g3Layer = dataStore.findLayerById('taipei_sn4_g');
  if (!f3Layer?.spaceNetworkGridJsonData?.length || !g3Layer) {
    console.warn('executeTaipeiTest4_F3_To_G3：缺少 f3 路網或 g3 圖層');
    return;
  }
  ...
  if (!g3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_g', { visible: true });
  }
...
/** g3：將 taipei_sn4_a 匯出列之中段站（黑點）依路線／路段序對應到 g3 折線，弧長均分 → 寫入 h3 */
export function executeTaipeiTest4_G3_To_H3() {
  const dataStore = useDataStore();
  const a3 = dataStore.findLayerById('taipei_sn4_a');
  const g3Layer = dataStore.findLayerById('taipei_sn4_g');
  const h3Layer = dataStore.findLayerById('taipei_sn4_h');
  if (!g3Layer?.spaceNetworkGridJsonData?.length || !h3Layer) {
    console.warn('executeTaipeiTest4_G3_To_H3：缺少 g3 路網或 taipei_sn4_h 圖層');
    return;
  }

  const check = assertA3RowsForG3ToH3(a3);
  if (!check.ok) {
    console.warn(`executeTaipeiTest4_G3_To_H3：${check.message}`);
    return;
  }
  ...
  h3Layer.dashboardData = {
    ...
    sourceLayerId: 'taipei_sn4_g',
    a3SourceLayerId: 'taipei_sn4_a',
  ...
  if (!h3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_h', { visible: true });
  }
}

/** h3：僅在既有頂點依黑點切段（不改座標、不重算紅黑站別、無線上權重數字）→ 寫入 taipei_sn4_i */
export function executeTaipeiTest4_H3_To_I3() {
  const dataStore = useDataStore();
  const h3Layer = dataStore.findLayerById('taipei_sn4_h');
  const i3Layer = dataStore.findLayerById('taipei_sn4_i');
  if (!h3Layer?.spaceNetworkGridJsonData?.length || !i3Layer) {
    console.warn('executeTaipeiTest4_H3_To_I3：缺少 h3 路網或 taipei_sn4_i 圖層');
    return;
  }
  ...
  i3Layer.layerInfoData = {
    splitSourceLayerId: 'taipei_sn4_h',
  ...
  i3Layer.dashboardData = {
    ...
    sourceLayerId: 'taipei_sn4_h',
    segmentsBeforeSplit: priorCount,
  };
  if (!i3Layer.visible) {
    i3Layer.visible = true;
    dataStore.saveLayerState('taipei_sn4_i', { visible: true });
  }
}

/** i3：讀 mrt_link_volume CSV → 對應切段 station_weights → 寫入 taipei_sn4_j（含 dataTableData） */
export async function executeTaipeiTest4_I3_To_J3() {
  const dataStore = useDataStore();
  const i3Layer = dataStore.findLayerById('taipei_sn4_i');
  const j3Layer = dataStore.findLayerById('taipei_sn4_j');
  if (!i3Layer?.spaceNetworkGridJsonData?.length || !j3Layer) {
    console.warn('executeTaipeiTest4_I3_To_J3：缺少 i3 路網或 taipei_sn4_j 圖層');
    return;
  }
  ...
  if (!trafficData?.rows?.length) {
    console.warn('executeTaipeiTest4_I3_To_J3：CSV 無資料或讀取失敗');
  }
  ...
  if (!j3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_j', { visible: true });
  }
}
...
/**
 * k3→l3：將 taipei_sn4_k 目前已載入之路網（含 K3 分頁專用欄位）與儀表板等深拷貝至 taipei_sn4_l（l3 另有獨立 jsonLoader／Vue 分頁，此函式僅負責複製）。
 */
export function executeTaipeiTest4_K3_To_L3() {
  const dataStore = useDataStore();
  const k3Layer = dataStore.findLayerById('taipei_sn4_k');
  const l3Layer = dataStore.findLayerById('taipei_sn4_l');
  if (!k3Layer || !l3Layer) {
    console.warn('executeTaipeiTest4_K3_To_L3：缺少 taipei_sn4_k 或 taipei_sn4_l 圖層');
    return;
  }
  const k3Routes = k3Layer.spaceNetworkGridJsonDataK3Tab;
  if (!Array.isArray(k3Routes) || k3Routes.length === 0) {
    console.warn(
      'executeTaipeiTest4_K3_To_L3：taipei_sn4_k 尚無 K3 分頁路網（請先載入 k3 之 JSON）'
    );
    return;
  }
  ...
  l3Layer.layerInfoData =
    baseInfo && typeof baseInfo === 'object'
      ? { ...baseInfo, copiedFromLayerId: 'taipei_sn4_k' }
      : { copiedFromLayerId: 'taipei_sn4_k' };
  ...
  if (!l3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_l', { visible: true });
  }

  dataStore.saveLayerState('taipei_sn4_l', {
...
/**
 * l3→m3：先依 dataStore 之 n 連續縮減黑點至「候選最小 weight 差 ≥ n」，再將 taipei_sn4_l 路網等深拷至 taipei_sn4_m。
 * ControlTab 會傳入 jsonData 作為第一參數；此流程改讀圖層 store，不使用該參數。
 */
export async function executeTaipeiTest4_L3_To_M3() {
  const dataStore = useDataStore();
  const l3Layer = dataStore.findLayerById('taipei_sn4_l');
  const m3Layer = dataStore.findLayerById('taipei_sn4_m');
  if (!l3Layer || !m3Layer) {
    console.warn('executeTaipeiTest4_L3_To_M3：缺少 taipei_sn4_l 或 taipei_sn4_m 圖層');
    return;
  }
  const l3Routes = l3Layer.spaceNetworkGridJsonDataL3Tab;
  if (!Array.isArray(l3Routes) || l3Routes.length === 0) {
    console.warn(
      'executeTaipeiTest4_L3_To_M3：taipei_sn4_l 尚無 L3 分頁路網（請先以 k3→l3 複製或載入 l3 之 JSON）'
    );
    return;
  }
  ...
  m3Layer.layerInfoData =
    baseInfo && typeof baseInfo === 'object'
      ? { ...baseInfo, copiedFromLayerId: 'taipei_sn4_l' }
      : { copiedFromLayerId: 'taipei_sn4_l' };
  ...
  if (!m3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_sn4_m', { visible: true });
  }

  dataStore.saveLayerState('taipei_sn4_m', {
