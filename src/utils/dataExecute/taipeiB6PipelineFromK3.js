...
/**
 * 版面網格測試_3：由 K3Tab flat segments 重算路網（可選零權重黑點合併）、mergeConnectSpans…、流量、權重 ÷100。
 * execute_A6_To_B6（寫入 taipei_b6）與手動合併（寫入 taipei_c6）共用。
 * （與 taipeiB5PipelineFromK3 分檔複製，測試_2／測試_3 不共用本體。）
 */
 ...
 * @param {object} srcA6 taipei_a6 圖層
 * @param {object} targetLayer 輸出圖層（taipei_b6 或 taipei_c6）
 * @param {Array} k3Routes 來源 K3Tab 路網（會深拷貝後正規化）
 * @param {{ zeroWeightMerge?: boolean, sourceLabel?: string }} options
 */
export async function runTaipeiB6PipelineFromK3Routes(
  dataStore,
  srcA6,
  targetLayer,
  k3Routes,
  ...
  // 手動合併（b6→c6）：
  //   來源 K3Tab 的黑點位置與 per-edge 權重皆已在 a6→b6 時定好；
  ...
  // a6→b6：仍跑完整 mergeConnectSpans…＋CSV＋÷100 流程。
  if (zeroWeightMerge) {
    finalSegs = segs;
  } else {
    const mapDrawnRowsRaw = extractMapDrawnRowsFromJson(srcA6.jsonData, srcA6.processedJsonData);
    ...
    if (mapDrawnRows?.length) {
      ...
      console.warn(
        `${options.sourceLabel || 'taipeiB6PipelineFromK3'}：缺少 mapDrawnRoutes 匯出列，改用原本黑點切段流程`
      );
  ...
  const trafficData = srcA6.trafficData;
  ...
  try {
    ...
    console.error(
      `${options.sourceLabel || 'taipeiB6PipelineFromK3'}：processedJsonData 轉換失敗`,
      e
    );
    processedJsonData = [];
  }
  ...
  targetLayer.jsonData = c(srcA6.jsonData);
  targetLayer.trafficData = c(srcA6.trafficData);
  ...
  const baseInfo = c(srcA6.layerInfoData);
  targetLayer.layerInfoData =
    baseInfo && typeof baseInfo === 'object'
      ? {
          ...baseInfo,
          copiedFromLayerId: 'taipei_a6',
  ...
  const baseDash = c(srcA6.dashboardData);
  ...
  targetLayer.drawJsonData = c(srcA6.drawJsonData);
  targetLayer.squareGridCellsTaipeiTest3 = srcA6.squareGridCellsTaipeiTest3 === true;
  targetLayer.showStationPlacement = srcA6.showStationPlacement !== false;
