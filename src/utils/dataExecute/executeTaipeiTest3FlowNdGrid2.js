...
import { buildTaipeiC3StyleLayerFieldsFromStraightenedNetwork } from '@/utils/taipeiTest3/buildTaipeiC3StyleLayerFieldsFromStraightenedNetwork.js';
...
import { buildTaipeiD3FromC3Network } from '@/utils/taipeiTest3/c3ToD3CoordNormalize.js';
import { applyHVToE3SpaceNetwork } from '@/utils/taipeiTest3/applyHVToE3Network.js';
import { applyNShapeReduceAllToSpaceNetworkData } from '@/utils/taipeiTest3/applyNShapeReduceToSpaceNetwork.js';
import { applyFlipLReduceF3ToG3Network } from '@/utils/taipeiTest3/f3ToG3FlipLReduce.js';
import { pruneGridLinesWithoutConnectVertices } from '@/utils/taipeiTest3/f3ToG3PruneEmptyGridLines.js';
import { applyG3ZShapeRedInteriorAxisSnapToNetwork } from '@/utils/taipeiTest3/g3ZShapeRedInteriorAxisSnapToNetwork.js';
import {
...
export function executeTaipeiTest3_G3_To_H3_Nd_Grid2() {
  ...
  const check = assertA3RowsForG3ToH3(b3ForRows);
  if (!check.ok) {
    console.warn(`executeTaipeiTest3_G3_To_H3_Nd：${check.message}`);
    return;
  }
...
export async function executeTaipeiTest3_I3_To_J3_Nd_Grid2() {
  ...
  if (!trafficData?.rows?.length) {
    console.warn('executeTaipeiTest3_I3_To_J3_Nd：CSV 無資料或讀取失敗');
  }
