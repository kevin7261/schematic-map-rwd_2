<script setup>
  /**
   * 📊 LayoutGridTab.vue - 版面網格數據視覺化分頁組件
   *
   * 功能說明：
  ...
  const getContainerId = () => {
    ...
    return `schematic-container-layout-grid-${layerId}`;
  };
   ...
   * 檢查圖層是否有任何可用的數據（dashboardData、layoutGridJsonData 等）
   */
  const currentLayerSummary = computed(() => {
    ...
    // 檢查是否有任何可用的數據（LayoutGridTab 只看 layoutGridJsonData）
    const hasData =
      (layer.dashboardData !== null && layer.dashboardData !== undefined) ||
      (layer.layoutGridJsonData !== null && layer.layoutGridJsonData !== undefined) ||
  ...
  /**
   * 📦 取得此分頁可用的主要示意圖資料
   * LayoutGridTab 只看 layoutGridJsonData
   */
  ...
  const loadLayerData = async (layerId) => {
    try {
      ...
      console.log(`[LayoutGridTab] 圖層資料檢查:`, {
        layerId,
        hasLayoutGridJsonData: !!targetLayer.layoutGridJsonData,
        layoutGridJsonDataType: targetLayer.layoutGridJsonData
          ? Array.isArray(targetLayer.layoutGridJsonData)
            ? 'Array'
            : typeof targetLayer.layoutGridJsonData
          : 'null',
        layoutGridJsonDataLength: Array.isArray(targetLayer.layoutGridJsonData)
          ? targetLayer.layoutGridJsonData.length
          : 'N/A',
      ...
      if (isMapLayer(layerId)) {
        ...
        // 如果有 layoutGridJsonData，優先檢查是否為 Normalize Segments 格式
        ...
        const layoutData = targetLayer.layoutGridJsonData;
  ...
  const drawMap = () => {
    ...
    // 不可因「尺寸相同」就提前 return：圖層 dataTableData／layoutGridJsonData 更新時仍須重繪，
    // 否則權重 label 永遠是舊 SVG，且不會跑到 dataTable 同步與缺漏檢查（會誤以為「程式沒改」）。
    ...
    if (isNormalizeFormat) {
      ...
      const resolveSegmentStationName = (seg, isEnd = false) => {
        const pick = (node) => {
          const raw =
            node?.station_name ?? node?.tags?.station_name ?? node?.tags?.name ?? node?.name ?? '';
          const s = String(raw).trim();
          return s === '—' || s === '－' ? '' : s;
        };

        // 優先以 points 端點 props 為準（與當前實際繪圖幾何同步），避免 properties_start/end 殘留舊值。
        const pts = Array.isArray(seg?.points) ? seg.points : [];
        if (pts.length > 0) {
          const p = isEnd ? pts[pts.length - 1] : pts[0];
          const pp = (Array.isArray(p) && p.length > 2 ? p[2] : null) || p;
          const fromPoint = pick(pp);
          if (fromPoint) return fromPoint;
        }

        const nodes = Array.isArray(seg?.nodes) ? seg.nodes : [];
        if (nodes.length > 0) {
          const node = isEnd ? nodes[nodes.length - 1] : nodes[0];
          const fromNode = pick(node);
          if (fromNode) return fromNode;
        }

        const direct = isEnd ? pick(seg?.properties_end) : pick(seg?.properties_start);
        if (direct) return direct;

        return '';
      };

      const routeSeqCounter = new Map();
      routeFeatures = flatSegments.map((seg) => {
        ...
        const segmentStartName = resolveSegmentStationName(seg, false);
        const segmentEndName = resolveSegmentStationName(seg, true);
        const routeName = seg.route_name || seg.name || '';
        const routeSeqKey = normalizeTaipeiK3WeightKeyPart(routeName);
        const currentSeq = (routeSeqCounter.get(routeSeqKey) || 0) + 1;
        routeSeqCounter.set(routeSeqKey, currentSeq);
    ...
    /** taipei_d3／e3／f3／g3：與 SpaceNetworkGridTab 一致，每 1 單位一條網格線 */
    const isTaipeiD3CoordNormalizeLayer =
      activeLayerTab.value === 'taipei_d3' ||
      activeLayerTab.value === 'taipei_sn4_d' ||
      activeLayerTab.value === 'taipei_e3' ||
      activeLayerTab.value === 'taipei_sn4_e' ||
      activeLayerTab.value === 'taipei_f3' ||
      activeLayerTab.value === 'taipei_sn4_f' ||
      activeLayerTab.value === 'taipei_g3' ||
      activeLayerTab.value === 'taipei_sn4_g' ||
      activeLayerTab.value === 'taipei_h3' ||
      activeLayerTab.value === 'taipei_sn4_h' ||
    ...
    // 粗網格步長為 5 的倍數；小範圍連續座標改以 5×10^k 步長產生刻度（與 SpaceNetworkGridTab 一致）
    ...
    const { lookup: routeSeqWeightLookupFromDataTable, conflicts: routeSeqWeightConflicts } =
      buildRouteSequenceWeightLookupFromTaipeiK3DataTable(activeLayerForWeights?.dataTableData);
    if (routeSeqWeightConflicts.length > 0) {
      console.warn(
        '[LayoutGridTab] dataTable 路段序權重有衝突，已採最後一筆覆蓋',
        routeSeqWeightConflicts.slice(0, 10)
      );
    }
    if (routeSeqWeightLookupFromDataTable.size > 0) {
      const missingRouteSeqKeys = [];
      for (const feature of routeFeatures) {
        const props = feature?.properties || {};
        const routeKey = normalizeTaipeiK3WeightKeyPart(props.name);
        const seq = Number(props.segment_route_seq);
        if (!routeKey || !Number.isFinite(seq)) continue;
        const key = `${routeKey}|${seq}`;
        if (!routeSeqWeightLookupFromDataTable.has(key)) {
          missingRouteSeqKeys.push(key);
        }
      }
      if (missingRouteSeqKeys.length > 0) {
        console.warn(
          '[LayoutGridTab] dataTable 缺少部分路段序權重，該段將不顯示權重',
          missingRouteSeqKeys.slice(0, 20)
        );
      }
    }
    ...
    const drawWeightLabelAt = (x, y, displayWeight) => {
      const textGroup = zoomGroup.append('g').attr('class', 'edge-weight-label');
      const padding = 0.1;
      const rectWidth = String(displayWeight).length * 4 + padding * 2;
    ...
    const drawRoutePath = (
      ...
      const routeKey = normalizeTaipeiK3WeightKeyPart(name);
      let segmentWeightFromDataTable = null;
      const segSeq = Number(segmentRouteSeq);
      if (routeKey && Number.isFinite(segSeq) && routeSeqWeightLookupFromDataTable.size > 0) {
        const seqKey = `${routeKey}|${segSeq}`;
        if (routeSeqWeightLookupFromDataTable.has(seqKey)) {
          segmentWeightFromDataTable = routeSeqWeightLookupFromDataTable.get(seqKey);
        }
      }

      let tooltipWeightsText = '';
      if (Number.isFinite(Number(segmentWeightFromDataTable))) {
        tooltipWeightsText = String(Number(segmentWeightFromDataTable));
      } else if (stationWeights && Array.isArray(stationWeights) && stationWeights.length > 0) {
        const ws = stationWeights
          .map((w) => Number(w?.weight))
          .filter((n) => Number.isFinite(n));
        if (ws.length) tooltipWeightsText = ws.join('，');
      }
      const startStationName = String(segmentStartName ?? '').trim();
      const endStationName = String(segmentEndName ?? '').trim();

      // 添加 hover 效果（含起迄站名、與畫面一致的權重：優先 dataTable，否則 station_weights）
      pathElement
        .on('mouseover', function (event) {
          ...
          let tooltipContent = '';
          if (name) {
            tooltipContent += `<strong>路線名稱:</strong> ${escapeTooltipText(name)}<br>`;
          }
          if (startStationName) {
            tooltipContent += `<strong>起站:</strong> ${escapeTooltipText(startStationName)}<br>`;
          }
          if (endStationName) {
            tooltipContent += `<strong>迄站:</strong> ${escapeTooltipText(endStationName)}<br>`;
          }
          if (tooltipWeightsText) {
            tooltipContent += `<strong>權重 (weight):</strong> ${tooltipWeightsText}<br>`;
          }
        ...
        .on('mouseout', function () {
          // 恢復路線樣式
          d3.select(this).attr('stroke-width', 3).attr('opacity', 0.9);

          // 隱藏 tooltip
          tooltip.style('opacity', 0);
        });

      // 優先使用 dataTableData 同步顯示：每條 segment 僅畫一次，避免同段重複 assign/覆蓋。
      if (Number.isFinite(Number(segmentWeightFromDataTable))) {
        ...
        const displayWeight = Number(segmentWeightFromDataTable);
        const textX = xScale(midX);
        const textY = yScale(midY);
        drawWeightLabelAt(textX, textY, displayWeight);
        return;
      }

      // 若已有 DataTable lookup，則顯示值只認 DataTable（對不到就不畫，避免錯值/重複 assign）。
      if (routeSeqWeightLookupFromDataTable.size > 0) {
        return;
      }

      // 回退：若無 dataTable 對應，才使用 station_weights
    ...
    routeFeatures.forEach((feature) => {
      ...
      if (geom.type === 'LineString') {
        drawRoutePath(
          ...
          props.points,
          props.segment_start_name,
          props.segment_end_name,
          props.segment_route_seq
        );
      } else if (geom.type === 'MultiLineString') {
    ...
    const gridDegKey = (gx, gy) => `${Math.round(Number(gx))},${Math.round(Number(gy))}`;
    let taipeiTest3ConnectDegreeMapLayout = null;
    let segmentsForTest3ConnectDegreeLayout = normalizeFlatSegments;
    if (isTaipeiTest3I3OrJ3LayerTab(activeLayerTab.value)) {
      ...
      if (Array.isArray(h3Data) && h3Data.length > 0) {
        try {
          segmentsForTest3ConnectDegreeLayout = normalizeSpaceNetworkDataToFlatSegments(h3Data);
        } catch {
          segmentsForTest3ConnectDegreeLayout = normalizeFlatSegments;
        }
      }
    }
    if (
      ...
      segmentsForTest3ConnectDegreeLayout.length > 0
    ) {
      taipeiTest3ConnectDegreeMapLayout = new Map();
      const bumpDeg = (k) =>
        taipeiTest3ConnectDegreeMapLayout.set(
          k,
          (taipeiTest3ConnectDegreeMapLayout.get(k) || 0) + 1
        );
      for (const seg of segmentsForTest3ConnectDegreeLayout) {
        const pts = seg.points || [];
        for (let i = 0; i < pts.length - 1; i++) {
          ...
          const k1 = gridDegKey(ax, ay);
          const k2 = gridDegKey(bx, by);
          if (k1 === k2) continue;
          bumpDeg(k1);
          bumpDeg(k2);
        }
    ...
    const taipeiTest3ConnectFillLayout = (isConn, lx, ly) => {
      if (!isConn || !taipeiTest3ConnectDegreeMapLayout) return null;
      const deg = taipeiTest3ConnectDegreeMapLayout.get(gridDegKey(lx, ly)) ?? 0;
    ...
    stationFeatures.forEach((feature) => {
      ...
      const fillColor =
        taipeiTest3ConnectFillLayout(isConnect, x, y) ?? (isConnect ? '#ff0000' : '#000000');
  ...
  /**
   * 👀 監聽當前圖層的主要示意圖資料變化
   * 當圖層的 layoutGridJsonData 載入完成時，自動載入並繪製
   */
  watch(
    () => {
      if (!activeLayerTab.value) return null;
