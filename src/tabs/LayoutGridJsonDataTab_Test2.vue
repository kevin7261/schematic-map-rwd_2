<script setup>
  /**
   * 📄 LayoutGridJsonDataTab_Test2.vue - 佈局網格 JSON 數據測試2分頁組件 (Layout Grid JSON Data Test2 Tab Component)
   *
   * 這是一個專門用於顯示圖層佈局網格 JSON 測試2數據的組件，提供多圖層的 JSON 數據查看功能。
   * 該組件能夠動態顯示當前開啟圖層的佈局網格 JSON 測試2數據，幫助用戶查看和檢查數據結構。
   *
   * 🎯 主要功能 (Core Features):
   * 1. 📄 佈局網格 JSON 測試2數據顯示：顯示圖層的佈局網格 JSON 測試2數據 (layoutGridJsonData_Test2)
   *    - 格式化 JSON 字符串顯示
   *    - 支持多圖層切換查看
   *    - 自動檢測數據可用性
   * 2. 🔄 動態圖層切換：支援多圖層的數據展示
   ...
   * - 研究工具的測試2數據檢查
   *
   * @component LayoutGridJsonDataTab_Test2
   * @version 1.0.0
   * @author Kevin Cheng
   * @since 1.0.0
   */
  ...
  const visibleLayers = computed(() => {
    ...
    return allLayers.filter((layer) => layer.visible && layer.layoutGridJsonData_Test2);
  });
  ...
  /**
   * 📊 當前圖層的佈局網格 JSON 測試2數據 (Current Layer Layout Grid JSON Test2 Data)
   */
  const currentLayerJsonData = computed(() => {
    if (!activeLayerTab.value) return null;
    const layer = visibleLayers.value.find((l) => l.layerId === activeLayerTab.value);
    return layer ? layer.layoutGridJsonData_Test2 || null : null;
  });
  ...
  <div class="d-flex flex-column my-bgcolor-gray-200 h-100">
    ...
    <div v-if="visibleLayers.length > 0" class="flex-grow-1 overflow-auto my-bgcolor-white">
      ...
      <div class="p-3 border-bottom">
        ...
        <div class="my-title-xs-gray">佈局網格 JSON 測試2數據 (layoutGridJsonData_Test2)</div>
      </div>
      ...
      <div v-else-if="currentLayerJsonData === null" class="p-3 text-center">
        <div class="my-title-md-gray py-5">此圖層沒有可用的佈局網格 JSON 測試2數據</div>
      </div>
    </div>
    ...
    <div v-else class="flex-grow-1 d-flex align-items-center justify-content-center">
      <div class="text-center">
        <div class="my-title-md-gray p-3">沒有開啟的圖層或沒有佈局網格 JSON 測試2數據</div>
      </div>
    </div>
  </div>
</template>
